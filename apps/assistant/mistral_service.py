...
class MistralAssistant:
    """AI Assistant using Mistral AI with farming domain boundaries"""
    
    def __init__(self):
        self.mistral_ai_url = settings.MISTRAL_AI_BASE_URL
        self.model = settings.MISTRAL_AI_MODEL
        
        # Compact system prompt for faster inference
        self.system_prompt = """You are FasAi, an expert Indian farming AI assistant.
RULES: Reply in the SAME language as the user (Hindi→Hindi, English→English, Hinglish→Hinglish). Only answer farming/agriculture questions. Be concise, use bullet points and emojis. Include dosage, prices, safety info when relevant. IMPORTANT: Write all words correctly — no garbled or broken words. For technical/scientific terms (pesticide names, chemical names, dosage units), always use proper English spelling (e.g. Mancozeb, Carbendazim, ml/liter, fungicide)."""
    ...
    def _build_messages(self, user_message, context=None):
        """Build messages for Mistral (only user/assistant roles supported)"""
        if context:
            context_text = "\n\n".join(context)
    ...
    def chat_stream(self, user_message, context=None):
        ...
        try:
            ...
            with requests.post(
                f"{self.mistral_ai_url}/v1/chat/completions",
                json={
                    "model": self.model,
                    "messages": messages,
                    "stream": True,
    ...
    def _get_fallback_response(self, message):
        """Fallback responses when Mistral AI is unavailable"""
        msg = message.lower()
        
        # Keyword-based responses
        if any(word in msg for word in ['wheat', 'gehun', 'गेहूं']):
            return {
                'response': """🌾 **गेहूं की खेती / Wheat Farming:**

**बुआई का समय:** अक्टूबर-नवंबर (Rabi season)
**Sowing Time:** October-November

**सिंचाई:** 4-6 बार (कुल 450-600mm)
**Irrigation:** 4-6 times (total 450-600mm)

**उर्वरक:** यूरिया 120kg + DAP 100kg प्रति एकड़
**Fertilizer:** Urea 120kg + DAP 100kg per acre

**मुख्य रोग:** Brown Rust, Yellow Rust
**Main Diseases:** Brown Rust, Yellow Rust

**नियंत्रण:** Propiconazole 25% EC @ ₹1,580/250ml
**Control:** Propiconazole 25% EC @ ₹1,580/250ml""",
                'is_farming_related': True,
                'has_context': False
            }
        
        elif any(word in msg for word in ['tomato', 'tamatar', 'टमाटर']):
            return {
                'response': """🍅 **टमाटर की खेती / Tomato Farming:**

**बुआई:** अगस्त-सितंबर और जनवरी-फरवरी
**Sowing:** August-September and January-February

**उर्वरक:** NPK 19:19:19 @ 2g/लीटर
**Fertilizer:** NPK 19:19:19 @ 2g/liter

**मुख्य रोग:** Early Blight, Late Blight
**Main Diseases:** Early Blight, Late Blight

**नियंत्रण:** Mancozeb 75% WP @ ₹1,480/500g
**Control:** Mancozeb 75% WP @ ₹1,480/500g

**सिंचाई:** प्रति सप्ताह 1-1.5 इंच
**Irrigation:** 1-1.5 inch per week""",
                'is_farming_related': True,
                'has_context': False
            }
